...
describe("NewSubmission router/handler integration tests", () => {
    ...
    describe("GET method`", () => {
        ...
        it.each(["en", "cy"])("Should redirect to the presenter journey with returnUrl containing submissionId (lang=\"%s\")", async (lang) => {
            ...
            const expectedRedirectUrl = `http://chs.local/transaction/${TRANSACTION_ID}/presenter?companyNumber=${COMPANY_NUMBER}&formType=VS01&returnUrl=${expectedReturnUrl}&lang=${lang}`;
