...
export const requestLogger = (req: Request, res: Response, next: NextFunction): void => {
    ...
    // Pre-process log
    logger.debugRequest(req, `${requestLogger.name} - OPEN request ${requestId}: ${req.method} ${req.originalUrl}`);

    // Post-process log
    res.on("finish", () => {
        const [seconds, nanoseconds] = process.hrtime(startTime);
        const durationInMs = (seconds * 1000 + nanoseconds / 1e6).toFixed(2);
        logger.debug(`${requestLogger.name} - CLOSED request ${requestId} after ${durationInMs}ms with status ${res.statusCode}`);
    });
